...
if (scalar(@ARGV) > 0) {
  ...
  my $FEATURE_TESTS = {
    't/modules/mod_statsd/sql.t' => {
      order => ++$order,
      test_class => [qw(mod_sql mod_sql_sqlite mod_statsd)],
    },

    't/modules/mod_statsd/tls.t' => {
      order => ++$order,
      test_class => [qw(mod_statsd mod_tls)],
    },
  };
